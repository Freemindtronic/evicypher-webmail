...
export const fetchKeys = async (
  ...
  const secondData: Record<string, string> = {
  ...
  if (exchange.link !== undefined) {
    ...
    secondData.ih = Base64.encode(ivd)
    secondData.sh = Base64.encode(saltd)
    secondData.dh = Base64.encode(encd)
  }

  const answer = await lanUtil.sendCipher(exchange.type, crypt.URL, secondData)

  const iv_low = utils.b64ToUint8Array(answer.i2)
  const salt_low = utils.b64ToUint8Array(answer.s2)
  const data_low_jam = utils.b64ToUint8Array(answer.d2)

  const iv_high = utils.b64ToUint8Array(answer.i)
  const salt_high = utils.b64ToUint8Array(answer.s)
  const data_high_jam = utils.b64ToUint8Array(answer.d)
