...
import { random, sha256, uint8ArrayToWordArray, wordArrayToUint8Array, } from '../utils';
import { AesUtil, shiftLeft, shiftRight } from './aes-util';
import { addJammingSimple, addJammingSimpleText, removeJammingSimple, removeJammingSimpleText, } from './jamming';
...
export class EviCrypt {
    ...
    async encryptText(plainText) {
        ...
        const enc = aes.encryptCTR(iv, salt, this.keys.high, new TextEncoder().encode(plainText));
        const jam = addJammingSimpleText(new Uint8Array([...iv, ...salt]), this.keys.low.slice(20), plainText.length);
        const idKeyFull = await sha256(new Uint8Array([...this.keys.low.slice(0, 20), ...jam.slice(0, 32)]));
    ...
    decryptText(cipherText) {
        ...
        let offset = 24;
        const dataToDecipher = removeJammingSimpleText(dataText.slice(offset), this.keys.low.slice(20), 48);
    ...
    async encryptFile(file, progressReporter) {
        ...
        while (j * blockSize < buffer.length) {
            const block = aes.encryptCTR(iv2, salt2, this.keys.high, buffer.slice(j * blockSize, (j + 1) * blockSize));
    ...
    async decryptFileBuffer(buffer, progressReporter) {
        if (buffer.length < 122 || // 4 + 20 + 16 + 32 + 16 + 32 + 2
        ...
        const idKeyFull = await sha256(new Uint8Array([...this.keys.low.slice(0, 20), ...saltId]));
        const idKey = idKeyFull.slice(0, 20);
        ...
        let offset = 73;
        const dataToDecipher = removeJammingSimple(buffer.slice(offset, offset + 1 + 515), this.keys.low.slice(20));
        offset += dataToDecipher.size;
        const aes = new AesUtil(256, 1000);
        const fileName = new TextDecoder().decode(aes.decryptCTR(iv, salt, this.keys.high, dataToDecipher.data.slice(48)));
        ...
        let j = 0;
        while (j * blockSize < buffer.length) {
            const block = aes.decryptCTR(iv2, salt2, this.keys.high, buffer.slice(offset + j * blockSize, offset + (j + 1) * blockSize));
            blobParts.push(block);
            progressReporter((j * blockSize) / buffer.length);
...
export const keyUsed = (str) => toUint8Array(str).slice(4, 56);
...
function incrementWordArray(arr, increment) {
...
//# sourceMappingURL=evicrypt.js.map