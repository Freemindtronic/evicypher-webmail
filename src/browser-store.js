...
export class BrowserStore {
     ...
     */
    constructor(name, writable, { fromJSON = (x) => x, toJSON = (x) => x, storage = browser.storage.local, } = {}) {
        this.name = name;
        ...
        BrowserStore.allLoaded = BrowserStore.allLoaded.then(async () => this.loadPromise);
        ...
        browser.storage.onChanged.addListener((changes, storageName) => {
            ...
            if (!sameStorage(storageName, storage) || !(this.name in changes))
                return;
            ...
            void Promise.resolve(fromJSON(JSON.parse(changes[this.name].newValue))).then((value) => {
                this.writable.set(value);
                loaded = true;
            });
        });
...
BrowserStore.allLoaded = Promise.resolve();
/** @returns Whether the storage given matches the name given */
const sameStorage = (storageName, storage) => (storageName === 'sync' && storage === browser.storage.sync) ||
