...
const loginRegex = /\b(login|user|username|loginusername|email|mail|phone|telephone|account)\b/i;
...
function processGroup(inputs) {
    ...
    if (inputs.length === 2) {
        if (Number(inputs[0].type === 'password') +
...
function findForms() {
    ...
    const inputs = [
        ...document.querySelectorAll('input[type=password], input[type=email], input[type=text], input[type=tel], input:not([type])'),
    ].filter((input) => input.style.display !== 'none');
    // Group these inputs by form
    const groups = new Map();
    for (const input of inputs) {
        if (!groups.has(input.form))
            groups.set(input.form, []);
        (_a = groups.get(input.form)) === null || _a === void 0 ? void 0 : _a.push(input);
    }
...
const icon = {
    focus: "url('data:image/png;base64,')",
...
function addButton(input) {
    ...
    const mouseoverHandler = (e) => {
        ...
        if (x >= threshold && !isFocus)
            focusButton();
        else if (x < threshold && isFocus)
            blurButton();
    };
...
const login = function () {
    return __asyncGenerator(this, arguments, function* () {
        ...
        yield yield __await(getHostname());
    });
};
/**
