            ...
            srcAllow.then((srcAllowResult) => {
                ...
                for (const frame of document.querySelectorAll('iframe')) {
                    ...
                    if (!srcAllowResultP.includes(frame.src) &&
                        ...
                        const warningURL = frame.src.length > 50 ? frame.src.slice(0, 50) + '...' : frame.src;
                        // Create URL element
                        ...
                        warningNever.addEventListener('click', (e) => {
                            e.preventDefault();
                            // Remove all warnings
                            for (const warningDiv of document.querySelectorAll('div.warning-' + rid))
                                warningDiv.remove();
                            // IFrame Styles
                            frame.style.filter = 'brightness(100%)';
                            frame.style.filter = 'initial';
                        ...
                        warningReadMore.addEventListener('click', (e) => {
                            e.preventDefault();
                            window.open('https://freemindtronic.com/bitb-browser-in-the-browser-evicypher-security-extension-for-web-browser', '_blank' // <- This is what makes it open in a new window.
                            );
                        });
                    }
...
// Mutation Observer
/// /////////////////////////////////////////////
MutationObserver = (_a = window.MutationObserver) !== null && _a !== void 0 ? _a : window.MutationObserver;
const observer = new MutationObserver((_mutations, _observer) => {
    if (get(isBITBEnabled))
        iframeFinder();
});
